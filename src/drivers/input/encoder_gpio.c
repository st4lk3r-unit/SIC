...
typedef struct {
    ...
    int last_a;         /* last sampled A level */
...
static int enc_read_delta(const void* self) {
    ...
    enc_gpio_t* e = (enc_gpio_t*)enc->impl;
...
static const struct encoder_s* make(const sic_encoder_cfg_t* cfg) {
    ...
    sic_gpio_mode_pullup(cfg->pin_a);
    sic_gpio_mode_pullup(cfg->pin_b);
    if (cfg->pin_btn != SIC_NOPIN)
        sic_gpio_mode_pullup(cfg->pin_btn);
    ...
    st->last_a = (sic_gpio_read(cfg->pin_a) ? 2 : 0)
               | (sic_gpio_read(cfg->pin_b) ? 1 : 0);
